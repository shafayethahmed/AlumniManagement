...
<div class="show-container">
    <div style="margin-bottom: 20px;">
        <a href="{{ route('user.member.alumni') }}" style="color: #64748b; text-decoration: none; font-size: 0.9rem;">
            ...
            <div class="data-item">
                ...
                @foreach ($member->experiences as $exp)
                    <div class="experience-entry">
                        <p class="job-title">
                            {{ $exp->position }}
                            {{-- Only show "Current" if they haven't resigned --}}
                            @if($exp->resign_at === null)
                                <span class="status-tag">Current</span>
                            @endif
                        </p>
                        <p class="company-name">{{ $exp->company }}</p>
                        <small style="color: #94a3b8;">
                            {{ $exp->started_at }} — {{ $exp->resign_at ?? 'Present' }}
                        </small>
                    </div>
                @endforeach
            </div>
